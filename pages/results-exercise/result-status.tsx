...
const ResultStatusPage = () => {
  ...
  return (
    <InnerLayout titlepage="ส่งผลสำเร็จ">
      <Box className={classes.root}>
        <Typography variant="h3">คุณใช้พลังงานไป</Typography>
        <Box className={classes.boxOpacity}>
          <Typography variant="h5">{sendExcer?.data?.calories || 0}</Typography>
        </Box>
        <Typography variant="body1">กิโลแคลอรี</Typography>
      </Box>
      <Box className={classes.boxStatus}>
        <Box className={classes.boxPoint}>
          <Typography variant="h5"> + {sendExcer?.data?.hp_gain || 0}</Typography>
          <Typography variant="body1">แต้ม</Typography>
        </Box>
        <Box className={classes.statusLevel}>
          ...
          <Box className={classes.imageLevel} style={{ marginLeft: -10 }}>
            <Box className={classes.icon}>
              <img src={`${prefix}/level/lv${profile?.level + 1 || 0}.svg`} alt="" />
            </Box>
            <Typography variant="body1">
              LEVEL &nbsp;<span>{profile?.level + 1 || 0}</span>
            </Typography>
          </Box>
        </Box>
        <Box
        ...
        <ControlButton justifycontent="center">
          ...
          />
          <ButtonProps
            titlebutton="ผลออกกำลังกาย"
            background="linear-gradient(to top, #8CA51E, #D0FD08)"
            borderradiusbtn="5px"
            boxshadowbtn="0 3px 6px rgba(0,0,0,0.16)"
            heightbtn="40px"
            fontsizebtn="22px"
            onClick={() => router.push('/results-exercise')}
          />
        </ControlButton>
      </Box>
    </InnerLayout>
...
export default ResultStatusPage;
